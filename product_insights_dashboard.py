...
def classify_product(mark: str, candidate_categories: list, threshold: int = 70) -> str:
    """
    Classify a product description (Mark) by fuzzy matching it against candidate categories.
    """
...
def product_insights_dashboard(data: pd.DataFrame):
    ...
    with tab_overview:
        ...
        fig_top = px.bar(top_products, x="Product", y="Tons",
    ...
    with tab_trends:
        ...
        fig_trends = px.line(trends_df, x="Period", y="Tons", color="Product",
                             title="Monthly Trends by Product Category", markers=True)
    ...
    with tab_market_share:
        ...
        fig_pie = px.pie(prod_agg, names="Product", values="Tons",
                         title="Product Category Market Share", hole=0.4)
        st.plotly_chart(fig_pie, use_container_width=True)
    
    with tab_details:
        st.subheader("Detailed Product Data")
        pivot_table = data.pivot_table(index="Product", columns="Period", values="Tons", aggfunc="sum", fill_value=0)
