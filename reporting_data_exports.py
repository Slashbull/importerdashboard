...
def export_to_csv(df: pd.DataFrame, columns: list, include_summary: bool) -> bytes:
    """
    Export selected columns to CSV, optionally prepending summary metrics.
    """
...
def export_to_excel(df: pd.DataFrame, columns: list, include_summary: bool) -> bytes:
    """
    Export selected columns to an Excel file with separate sheets for data and summary.
    """
    output = io.BytesIO()
...
def export_to_pdf(df: pd.DataFrame, columns: list, include_summary: bool) -> bytes:
    ...
    try:
        ...
        st.error("⚠️ PDF export requires 'pdfkit'. Install via pip install pdfkit")
        return None
    html = """
    <html>
      <head>
