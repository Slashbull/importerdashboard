...
def competitor_intelligence_dashboard(data: pd.DataFrame):
    ...
    with tab_top:
        ...
        top_n = st.selectbox("Select number of top competitors to display:", [5, 10, 15, 20, 25], index=0)
        top_competitors = data.groupby("Consignee")["Tons"].sum().nlargest(top_n).reset_index()
        fig_top = px.bar(top_competitors, x="Consignee", y="Tons",
    ...
    with tab_export:
        ...
        fig_export = px.bar(exporter_breakdown, x="Exporter", y="Tons",
