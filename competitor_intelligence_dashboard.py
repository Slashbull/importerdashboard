...
# -----------------------------------------------------------------------------
# Helper functions for UI/UX enhancements
# -----------------------------------------------------------------------------
...
def logout_button():
    """Display a logout button to clear the session and refresh the app."""
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Month" in df.columns and "Year" in df.columns:
        try:
            ...
            st.error("Error parsing 'Month' and 'Year'. Ensure proper format.")
...
def upload_data():
    """
    Handle data upload from CSV or Google Sheets, then preprocess and store data in session state.
    """
...
# -----------------------------------------------------------------------------
# UI/UX Enhancements: Custom CSS and Header/Footer
# -----------------------------------------------------------------------------
def add_custom_css():
    custom_css = """
    <style>
        ...
        /* Main content margin to avoid overlap with fixed header */
        .main-content {
            margin-top: 70px;
        }
        /* Sidebar styling for navigation and filters */
...
def main():
    ...
    # Use sidebar radio for navigation between pages
    nav_options = [
        "Home",
        "Market Overview",
    ...
    # Sidebar: Advanced Filters (collapsible expander)
    with st.sidebar.expander("Advanced Filters", expanded=False):
        st.write("Filters will be applied after data is loaded.")
    
    # Authentication & Logout
    authenticate_user()
    logout_button()
    
    # Routing logic based on selected page
    if selected_page == "Home":
        ...
        if df is not None and not df.empty:
            ...
            st.info("Please upload your data to see insights.")
