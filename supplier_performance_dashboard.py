...
def supplier_performance_dashboard(data: pd.DataFrame):
    ...
    tab_kpis, tab_top, tab_trends, tab_importers = st.tabs(["Key Metrics", "Top Suppliers", "Trends & Risk Analysis", "Importer Connections"])
    ...
    with tab_top:
        ...
        top_n = st.selectbox("Select number of top suppliers to display:", [5, 10, 15, 20, 25], index=0)
        top_suppliers = supplier_agg.nlargest(top_n, "Tons")
        fig_top = px.bar(top_suppliers, x="Exporter", y="Tons",
    ...
    with tab_importers:
        ...
        fig_tree = px.treemap(contrib, path=["Exporter", "Consignee"], values="Tons",
