...
def supplier_performance_dashboard(data: pd.DataFrame):
    ...
    top_suppliers = data.groupby("Exporter")["Tons"].sum().nlargest(5).reset_index()
    fig1 = px.bar(top_suppliers, x="Exporter", y="Tons", title="Top Suppliers")
    st.plotly_chart(fig1, use_container_width=True)

    st.markdown("### 📈 Supplier Consistency Analysis")
    data["Period"] = data["Month"] + "-" + data["Year"].astype(str)
    supplier_trends = data.groupby(["Exporter", "Period"])["Tons"].sum().unstack(fill_value=0)
