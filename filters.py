...
def apply_filters(df: pd.DataFrame):
    ...
    # If the column exists, get all unique values; otherwise, use an empty list.
    states = df["Consignee State"].unique().tolist() if "Consignee State" in df.columns else []
    selected_state = st.sidebar.multiselect("📌 Select State:", options=states, default=states)
    
    months = df["Month"].unique().tolist() if "Month" in df.columns else []
    selected_month = st.sidebar.multiselect("📅 Select Month:", options=months, default=months)
    ...
    selected_consignee = st.sidebar.multiselect("🏢 Select Consignee:", options=consignees, default=consignees)
    
    exporters = df["Exporter"].unique().tolist() if "Exporter" in df.columns else []
    selected_exporter = st.sidebar.multiselect("🚢 Select Exporter:", options=exporters, default=exporters)
    
    marks = df["Mark"].unique().tolist() if "Mark" in df.columns else []
    selected_mark = st.sidebar.multiselect("🔖 Select Mark:", options=marks, default=marks)
    
    unit_toggle = st.sidebar.radio("⚖️ Select Unit:", ("Kgs", "Tons"))
    unit_column = "Kgs" if unit_toggle == "Kgs" else "Tons"
    
    # Apply filters if the corresponding column exists.
    filtered_df = df.copy()
